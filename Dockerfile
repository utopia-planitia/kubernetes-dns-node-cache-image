# renovate: datasource=repology depName=alpine_3_16/iptables
ARG IPTABLES_VERSION=1.8.8-r1
...
RUN set -eux; \
    ...
    cp -r /etc/apk/keys /etc/apk/repositories /rootfs/etc/apk; \
    apk add \
        --initdb \
        --no-cache \
        --root /rootfs \
...
# TODO: find out how to run / innstall https://github.com/kubernetes-sigs/iptables-wrappers in /rootfs (without a shell and without coreutils) instead of changing the symlinks
RUN set -eux; \
...
# copy the artifacts from the previous stages into the final distroless / scratch image
